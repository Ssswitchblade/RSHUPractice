...
public class Mr231_3Converter implements SearadarExchangeConverter {
    ...
    ppublic List<SearadarStationMessage> convert(String message) {
        ...
        switch (msgType) {
            case "TTM": 
                try {
                    msgList.add(parseTTM());
                } catch (IllegalArgumentException e) {
                    System.err.println("Ошибка при обработке сообщения TTM: " + e.getMessage());
                }
                break;
            case "RSD": {
                try {
                    ...
                    System.err.println("Ошибка при обработке сообщения RSD: " + e.getMessage());
                }
                break;
            }
        }
